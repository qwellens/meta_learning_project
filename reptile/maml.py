...
from utils import mse, xent, conv_block, normalize, reptile
...
class MAML:
    ...
    def construct_model(self, input_tensors=None, prefix='metatrain_'):
        ...
        with tf.variable_scope('model', reuse=None) as training_scope:
            ...
            def task_metalearn(inp, reuse=True):
                ...
                for j in range(num_updates - 1):
                    #only used if you wanna update gradients more than once on same K points
        ...
        if 'train' in prefix:
            ...
            #only used when not doing metalearning
            self.pretrain_op = tf.train.AdamOptimizer(self.meta_lr).minimize(total_loss1)

            if FLAGS.metatrain_iterations > 0 and not FLAGS.reptile:
